...
// Gravity Physics
const N  = 150 // No. of bodies
const G = 100 // Gravity strength
const eps = 0.05 // time step
...
func (this Body) getGravity(Bodies [N]Body) Vector{
	...
	for j:= 0; j<gn ; j++{
		...
		for i := 0; i < N; i++ {
			if Bodies[i].group == j {
				cms[j].Add(Bodies[i].pos.Scale(Bodies[i].mass))
				sum = sum + Bodies[i].mass
				prod = prod * Bodies[i].mass
			}
		rms[j] = prod/sum
		}
	}
	for i := 0; i < N; i++ {
		Bodies[i].Com = cms[Bodies[i].group]
		Bodies[i].mu = rms[Bodies[i].group]
	}

	var r = this.Com.Add(this.pos.Neg()).Norm()
	this.acc = (this.Com.Add(this.pos.Neg()).Scale((G ) / (r * r * r))).Add(this.acc)
	return this.acc
}
...
func run() {
	...
	for i := 0; i<N/2; i++ {
		c := 26.
		Bodies[i] = Body{rand.Float64()*10 + 2, Vector{rand.Float64()*metric, rand.Float64()*metric-metric/2}, Vector{rand.Float64()*c-c/2,rand.Float64()*c-c/2}, Vector{0,0}, 1.0, 0, Vector{0,0}, 0}
	}
	for i := N/2+1; i<N; i++ {
		c := 26.
		Bodies[i] = Body{rand.Float64()*10 + 2, Vector{rand.Float64()*metric, rand.Float64()*metric-metric/2}, Vector{rand.Float64()*c-c/2,rand.Float64()*c-c/2}, Vector{0,0}, 1.0, 1, Vector{0,0}, 0}
	}
	// Fps Setup
	var (
		frames = 0
	...
	for !win.Closed() {
		...
		for i:=0;i<N;i++{
			...
			mat := pixel.IM
			mat = mat.Moved(pixel.V(x,y)).Scaled(pixel.V(x,y),zoom).Scaled(pixel.V(x,y),Bodies[i].mass/4).Moved(pixel.V(camPos.x,camPos.y)).Scaled(pixel.V(winWidth/2,winHeight/2),camZoom)
