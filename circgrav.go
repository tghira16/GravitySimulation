...
// Gravity Physics
const N  = 400 // No. of bodies
const G = 1 // Gravity strength
const eps = 0.02 // time step
...
func run() {
	...
	for i := 0; i<N; i++ {
		c := 40.
		var x = rand.Float64()*4*pi - 2*pi
		Bodies[i] = Body{rand.Float64()*40 + 10, Vector{math.Sin(x)*metric-metric/2, math.Cos(x)*metric-metric/2}, Vector{rand.Float64()*c-c/2,rand.Float64()*c-c/2}, Vector{0,0}, 1.0}
	}
